...
public class FaceAlignProc{
	...
	public void drawTestInfo(Canvas canvas, float xScale, float yScale, float translateX, float translateY){
		FuncTracer.startFunc();
		...
		Log.v("TANGZHIMING", "DrawTestInfo: mScaleFactor="+mScaleFactor+" xScale="+xScale+" yScale="+yScale+" translateX="+translateX+ " translateY="+translateY);
		if (test_PlotPatch){
			Paint pt = new Paint();
			...
			for (int i=0; i<mModel.numPts; i++){
				...
				for (int j=0; j<SEARCH_WIN_H+mModel.patchModel.sampleHeight-1; j++){
					for (int k=0; k<(SEARCH_WIN_W+mModel.patchModel.sampleWidth-1); k++){
						...
						left = left*xScale+translateX;
						top = top*yScale+translateY;
						right = right*xScale+translateX;
						bottom = bottom*yScale+translateY;
			...
			for (int i=0; i<path.paths.length; i++){
				...
				if (test_PlotOriginal){
					...
					if (1 == path.paths[i].length){
						...
						centX = centX*xScale+translateX;
						centY = centY*yScale+translateY;
						...
						for (int j=1; j<path.paths[i].length; j++){
							...
							startX = startX*xScale+translateX;
							endX = endX*xScale+translateX;
							startY = startY*yScale+translateY;
							endY = endY*yScale+translateY;
